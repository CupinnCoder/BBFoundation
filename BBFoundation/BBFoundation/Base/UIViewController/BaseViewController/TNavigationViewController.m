...
@property (nonatomic ,strong) BBDirectionalPanGestureRecognizer                 *panRecognizer;
...
@property (nonatomic, assign) BBNavigationViewControllerNavigationBarVisibility navigationBarVisibility;
...
- (id)initWithRootViewController:(UIViewController *)rootViewController {
    ...
    if(self) {
        ...
        [[TNavigationBar appearance] setTitleTextAttributes:
         @{ NSForegroundColorAttributeName: BBSTYLEVAR(navBarTitleColor),
            NSFontAttributeName: BBSTYLEVAR(navBarTitleFont)}];

        [[TNavigationBar appearance] setBarBgColor:BBSTYLEVAR(navBarBackgroundColor)];
        [[TNavigationBar appearance] setTintColor:BBSTYLEVAR(navBarBackgroundColor)];
    }
...
- (void)viewDidLoad
{
    ...
    if (_canDragBack) {
        if (!_panRecognizer) {
            _panRecognizer = [[BBDirectionalPanGestureRecognizer alloc]
                              initWithTarget:self
                              action:@selector(paningGestureReceive:)];
            _panRecognizer.direction = BBPanDirectionRight;
    ...
    self.navigationBarVisibility = BBNavigationViewControllerNavigationBarVisibilitySystem;
...
- (void)setNavigationBarVisibility:(BBNavigationViewControllerNavigationBarVisibility)navigationBarVisibility
{
    if (_navigationBarVisibility == navigationBarVisibility) return;
    
    if (_navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilitySystem) {
        if (navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityHidden ||
            navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityVisible) {
//            [self transitionFromSystemNavigationBarToCustom];
        }
    } else if (_navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityHidden ||
               _navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityVisible) {
        if (navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilitySystem) {
//            [self transitionFromCustomNavigationBarToSystem];
        }
    }
    
    if (navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityUndefined) {
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    if (self.navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityHidden) {
...
{
    if ([viewController conformsToProtocol:@protocol(BBNavigationViewControllerDelegate)]) {
        self.navigationBarVisibility = (BBNavigationViewControllerNavigationBarVisibility)[viewController performSelector:@selector(preferredNavigationBarVisibility)];
    } else {
        self.navigationBarVisibility = BBNavigationViewControllerNavigationBarVisibilitySystem;
    }
    
    if (self.navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityVisible ||
        self.navigationBarVisibility == BBNavigationViewControllerNavigationBarVisibilityHidden) {
...
- (void)showCustomNavigaitonBar:(BOOL)show withFadeAnimation:(BOOL)animated
{
    [UIView animateWithDuration:(animated ? 0.2 : 0) animations:^{
        ...
        [self setNeedsStatusBarAppearanceUpdate];
        self.navigationBarVisibility = show ? BBNavigationViewControllerNavigationBarVisibilityVisible : BBNavigationViewControllerNavigationBarVisibilityHidden;
    }];
}
...
{
    if ([self.topViewController conformsToProtocol:@protocol(BBNavigationViewControllerDelegate)]) {
        BBNavigationViewControllerNavigationBarVisibility topControllerPrefersVisibility = (BBNavigationViewControllerNavigationBarVisibility)[self.topViewController performSelector:@selector(preferredNavigationBarVisibility)];
        
        if (topControllerPrefersVisibility == BBNavigationViewControllerNavigationBarVisibilityVisible) {
            [self showCustomNavigaitonBar:YES withFadeAnimation:animated];
        } else if (topControllerPrefersVisibility == BBNavigationViewControllerNavigationBarVisibilityHidden) {
