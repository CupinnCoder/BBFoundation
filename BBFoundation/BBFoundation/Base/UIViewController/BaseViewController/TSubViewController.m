...
- (void)loadView {
    if (nil != self.nibName) {
        ...
        self.view.backgroundColor = BBSTYLEVAR(viewBackgroundColor);
    }
}
...
- (void)viewDidLoad
{
    ...
    self.navigationController.view.backgroundColor = BBSTYLEVAR(viewBackgroundColor);
}
...
- (BBStateView *)stateView {
    if (!_stateView) {
        _stateView = [[BBStateView alloc] initWithFrame:self.view.bounds];
...
- (NSString *)stateViewTitle:(NSInteger)viewState {
    NSString *title = @"";
    if (viewState == kBBViewStateDataError) {
        title = NSLocalizedString(@"网络数据异常", nil);
    }
    if (viewState == kBBViewStateLoading) {
        title = NSLocalizedString(@"正在加载数据", nil);
    }
    if (viewState == kBBViewStateNetError) {
        title = NSLocalizedString(@"网络连接错误", nil);
    }
    if (viewState == kBBViewStateNoData) {
        title = NSLocalizedString(@"网络数据为空", nil);
    }
    if (viewState == kBBViewStateTimeOut) {
...
- (UIImage*)buttonBackgroundImageForStateView:(UIView *)view forState:(UIControlState)state {
    UIImage *image = nil;
    image = [[BBCoreUtility sharedUtility] createImageWithColor:[UIColor whiteColor] width:236 / 2 height:30];
...
- (NSAttributedString *)titleForStateView:(UIView *)view {
    ...
    NSDictionary *attributes = @{NSFontAttributeName:BBSTYLEVAR(loadingTexBBont),
                                 NSForegroundColorAttributeName:BBSTYLEVAR(loadingTextColor)};
...
- (NSAttributedString *)buttonTitleForStateView:(UIView *)view forState:(UIControlState)state {
    ...
    NSDictionary *attributes = @{NSFontAttributeName:BBSTYLEVAR(font16),
                                 NSForegroundColorAttributeName:BBSTYLEVAR(loadingTextColor)};
...
- (FLAnimatedImage *)animationImageForStateView:(UIView *)view {
    if (_viewState == kBBViewStateLoading) {
...
- (UIColor *)backgroundColorForStateView:(UIView *)view {
    return BBSTYLEVAR(viewBackgroundColor);
   }

- (CGFloat)spaceHeighBBorStateView:(UIView *)view {
    return 12;
}
...
-(void) checkGuide {
    ...
    //取出当前页面 的引导信息
    ViewGuideModel *model = [[BBDataHelper shared] loadViewGuideWithViewId:viewId];
    if (!model) {
        BBLog(@"（%@）页面未找到引导信息！",viewId);
        return;
    }
    //引导 开始
