...
@interface BBWebViewController ()<NJKWebViewProgressDelegate>{
    BOOL            firstLoaded;
...
- (void)viewDidLoad
{
    ...
    self.webView.top = 64.f;
    self.webView.height -= 64.f;
    
    self.navigationItem.rightBarButtonItems = nil;
    self.navigationItem.leftBarButtonItems = [[BBCoreUtility sharedUtility] createBarButtonsWithImage:@"NavButtonBack.png"
    ...
    switch (_shareType) {
        ...
        case LocalViewTypeEventDetail:
            self.navigationItem.rightBarButtonItems = [[BBCoreUtility sharedUtility] createBarButtonsWithImage:@"NavButtonMore.png"
...
- (void)addObserver {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleNotice:)
                                                 name:kBBCloseWebViewNotification
                                               object:nil];
    
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleNotice:)
                                                 name:kBBOpenLocalNotification
                                               object:nil];
}

- (void)removeObserver {
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kBBCloseWebViewNotification object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kBBOpenLocalNotification object:nil];
...
//    ShareBBType shareType = ShareBBTypeWeb;
//    NSString *title = [shareContent objecBBorKey:@"title"]?[shareContent objecBBorKey:@"title"]:self.navigationItem.title;
//    NSString *content = [shareContent objecBBorKey:@"content"]?[shareContent objecBBorKey:@"content"]:self.navigationItem.title;
//    NSString *url = [shareContent objecBBorKey:@"url"]?[shareContent objecBBorKey:@"title"]:_url;
//    NSString *imageUrl = @"";
//    if ([[shareContent objecBBorKey:@"icon"] length]) {
//        imageUrl = [shareContent objecBBorKey:@"icon"];
//    }
//    switch (_shareType) {
//        case LocalViewTypeEventDetail:
//            shareType = ShareBBTypeEvent;
//            break;
//            
//        default:
//            break;
...
- (void)handleNotice:(NSNotification *)notice {
    if ([notice.name isEqualToString:kBBCloseWebViewNotification]) {
        //关闭
        [self closeAction];
    }
    if ([notice.name isEqualToString:kBBOpenLocalNotification]) {
        //打开本地页面
