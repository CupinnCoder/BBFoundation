...
- (BOOL)isJailbroken {
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString bb_stringWithUUID]];
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} bb_net_interface_counter;


static uint64_t bb_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t bb_net_counter_get_by_type(bb_net_interface_counter *counter, BBNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & BBNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & BBNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & BBNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & BBNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & BBNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & BBNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static bb_net_interface_counter bb_get_net_interface_counter() {
    ...
    bb_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = bb_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = bb_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)bb_getNetworkTrafficBytes:(BBNetworkTrafficType)types {
    bb_net_interface_counter counter = bb_get_net_interface_counter();
    return bb_net_counter_get_by_type(&counter, types);
}
