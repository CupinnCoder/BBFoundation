...
- (void)genieInTransitionWithDuration:(NSTimeInterval)duration destinationRect:(CGRect)destRect destinationEdge:(BCRectEdge)destEdge scal:(CGFloat)scale completion:(void (^)())completion {
    
    [self genieTransitionWithDuration:duration
                                 edge:destEdge
                      destinationRect:destRect
                              reverse:NO scale:scale
...
- (void) genieTransitionWithDuration:(NSTimeInterval) duration
                     ...
                     destinationRect:(CGRect)destRect
                             reverse:(BOOL)reverse scale:(CGFloat)scale
                          completion:(void (^)())completion
{
    ...
    UIImage *snapshot = [self renderSnapshotWithMarginForAxis:axis scale:scale];
...
- (UIImage *) renderSnapshotWithMarginForAxis:(BCAxis)axis scale:(CGFloat)scale
{
    CGSize contextSize = self.frame.size;
//    TFLog(@"transform.a = %f b = %f, c = %f, d = %f",self.transform.a,self.transform.b,self.transform.c,self.transform.d);
//    
//    CGSize contextSize = CGSizeMake(self.frame.size.width * scale * .4f, self.frame.size.height * scale *.4f);
