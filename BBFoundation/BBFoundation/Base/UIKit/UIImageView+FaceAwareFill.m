        ...
        @autoreleasepool {
            ...
            if (features.count > 0) {
                for (CIFaceFeature* faceFeature in features) {
                    BBLog(@"features:%@",NSStringFromCGRect(faceFeature.bounds));
                    if (faceFeature.bounds.size.width > 0 && faceFeature.bounds.size.height > 0) {
                        UIView *view = [[UIView alloc] initWithFrame:faceFeature.bounds];
                        view.layer.borderWidth = 1;
                        view.layer.borderColor = [[UIColor redColor] CGColor];
                        [self addSubview:view];
//                        [array addObject:NSStringFromCGRect(faceFeature.bounds)];
                    }
                }
            }
        }
