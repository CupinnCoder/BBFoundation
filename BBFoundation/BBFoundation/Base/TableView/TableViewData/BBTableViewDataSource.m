...
#import <MyBBTableViewManager/MyBBTableViewLoadingItem.h>
#import <MyBBTableViewManager/MyBBTableViewLoadingItemCell.h>


@interface BBTableViewDataSource()<RETableViewManagerDelegate,MyBBTableViewManagerDelegate> {
...
- (void)load:(DataLoadPolicy)loadPolicy params:(NSDictionary *)params context:(ASBatchContext *)context {
    BBLog(@"------------------------------load _loading = %@ dataLoadPolicy = %@",@(_loading),@(loadPolicy));
    if (_loading) {
        return;
    }
    ...
    void(^handleTableViewData)(id result,DataLoadPolicy dataLoadPolicy,NSError *hanldeError) = ^(id result,DataLoadPolicy dataLoadPolicy,NSError *hanldeError) {
        BBLog(@"----------------------------------handleTableViewData  loadPolicy = %@",@(dataLoadPolicy));
        typeof(self) strongSelf = weakSelf;
                 ...
                 dispatch_async(dispatch_get_main_queue(), ^{
                     ...
                     if (_currentPage < _totalPage) {
                         ...
                         if (_managerFlag) {
                             ...
                             [section addItem:[MyBBTableViewLoadingItem itemWithTitle:NSLocalizedString(@"正在加载...", nil)]];
    ...
    [[NetworkAssistant sharedAssistant] getDataByURL:url
                                            ...
                                            fileData:nil
                                               ...
                                               start:^(id cacheResult){
                                                   _loading = YES;
                                                   if (!_buildingView) {
                                                       if (loadPolicy == DataLoadPolicyCache && _useCacheData) {                                                       BBLog(@"show cache data===============");
 ...
 */
- (void)registerClass {
    NSArray *tableViewItemlist = [CLClassList subclassesOfClass:[BBTableViewItem class]];
    if (_managerFlag) {
        tableViewItemlist = [CLClassList subclassesOfClass:[BBTableViewItem class]];
    }
...
- (NSInteger)detectScrollDirection:(CGFloat)currentOffsetY previousOffsetY:(CGFloat)previousOffsetY {
    return currentOffsetY > previousOffsetY ? kBBScrollDirectionUp   :
    currentOffsetY < previousOffsetY ? kBBScrollDirectionDown :
    kBBScrollDirectionNone;
}
...
- (BOOL)shouldBatchFetchForTableView:(ASTableView *)tableView {
    BBLog(@"shouldBatchFetchForTableView");
...
- (void)tableView:(ASTableView *)tableView willBeginBatchFetchWithContext:(ASBatchContext *)context {
    BBLog(@"willBeginBatchFetchWithContext");
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    BOOL isBouncing = (isOverTopBoundary && currentScrollDirection != kBBScrollDirectionDown) || (isOverBottomBoundary && currentScrollDirection != kBBScrollDirectionUp);
    ...
    if (currentScrollDirection == kBBScrollDirectionUp) {
    ...
    else if (currentScrollDirection == kBBScrollDirectionDown) {
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    if (_previousScrollDirection == kBBScrollDirectionUp) {
        BOOL isOverThreshold = _accumulatedY < -_upThresholdY;
        BOOL isOverBottomBoundary = currentOffsetY >= bottomBoundary;
    ...
    else if (_previousScrollDirection == kBBScrollDirectionDown) {
        BOOL isOverThreshold = _accumulatedY > _downThresholdY;
        BOOL isOverTopBoundary = currentOffsetY <= topBoundary;
