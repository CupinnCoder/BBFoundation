...
@implementation BBCoreUtility


+ (instancetype)sharedUtility
{
    static BBCoreUtility* utility = nil;
...
- (void)addBookMark:(NSString *)dataId index:(NSUInteger)index isMark:(BOOL)isMark {
    [SSKeychain setPassword:[NSString stringWithFormat:@"%lu",(unsigned long)(isMark?index:0)]
                 forService:[NSString stringWithFormat:@"cupinn_ios_book_mark_%@",dataId]
                    account:@"app_user"];
...
//                if (error) {
//                    BBLog(@"delete file error:%@",[error debugDescription]);
//                }
//                //移动文件
...
- (NSUInteger)getBookMark:(NSString *)dataId {
    NSString *index = [SSKeychain passwordForService:[NSString stringWithFormat:@"cupinn_ios_book_mark_%@",dataId]
                                             account:@"app_user"];
    if (index) {
