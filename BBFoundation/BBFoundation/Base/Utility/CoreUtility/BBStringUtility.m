...
@implementation BBStringUtility

+(instancetype)shared {
    static BBStringUtility* utility = nil;
...
-(NSString *)showUsername:(NSString *)name {
    NSString *username = @"";
    if ([[BBCoreUtility sharedUtility] validateUserPhone:name]) {
...
- (NSString *)getTimeStringByTimeInterval:(NSTimeInterval)timeInterval {
    NSDate *fromdate= [[BBCoreUtility sharedUtility] getDateByTimeInterval:timeInterval];
...
- (NSString *)subString:(NSString *)str length:(NSInteger)length {
    ...
    if ([[BBCoreUtility sharedUtility] charNumber:str] > length) {
        NSString* tmp;
        for (NSInteger i = str.length; i > 0; i--) {
            tmp = [str substringToIndex:i];
            if ([[BBCoreUtility sharedUtility] charNumber:tmp] <= length) {
                return tmp;
                break;
            }
        }
...
-(NSString *) shareMessageWithString:(NSString *)string maxLength:(NSUInteger)maxLength {
    NSUInteger len = [[BBCoreUtility sharedUtility] unicodeLengthOfString:string];
    if (len > maxLength) {
