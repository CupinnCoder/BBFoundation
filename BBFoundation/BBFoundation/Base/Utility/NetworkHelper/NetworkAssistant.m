...
- (void)saveImageIndex {
    ...
    NSURLSessionUploadTask *uploadTask = [manager uploadTaskWithStreamedRequest:request
                                          ...
                                          {
                                              BBLog(@"responseObject:%@",responseObject);
                                          }];
...
- (void)handleByURL:(NSString *)url
    ...
    NSString *cacheKey = [[BBCoreUtility sharedUtility] getMD5StringFromNSString:[url stringByAppendingString:params?[params description]:@""]];
    BBLog(@"cacheKey:%@",cacheKey);
    ...
    BBLog(@"request header = %@ url = %@",manager.requestSerializer.HTTPRequestHeaders,url);
    BBLog(@"params = %@",params);
    ...
    if ([[Reachability reachabilityForInternetConnection] currentReachabilityStatus] == NotReachable) {
        //网络异常
        NSError *error = [NSError errorWithDomain:BB_APP_ERROR_DOMAIN
                                             code:kBBErrorCodeNetwork
                                         userInfo:nil];
...
- (void)postSuccess:(NSURLSessionTask *)task
    ...
    @autoreleasepool {
        if (!rootObject) {
            error = [NSError errorWithDomain:BB_APP_ERROR_DOMAIN
                                        code:kBBErrorCodeAPI
                                    userInfo:@{@"info":@"数据解析错误"}];
        } else {
            NSInteger status = [[rootObject objectForKey:@"status"] boolValue];
            BBLog(@"errorcode = %@",[rootObject objectForKey:@"errorCode"]);
            NSInteger code = [[rootObject objectForKey:@"errorCode"] integerValue];
            if (!status && code != kBBErrorCodeUnknown) {
                error = [NSError errorWithDomain:BB_APP_ERROR_DOMAIN
                                            code:code
                                        userInfo:nil];
            }
