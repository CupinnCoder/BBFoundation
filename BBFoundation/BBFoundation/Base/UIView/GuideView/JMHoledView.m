...
- (void)setup
{
    ...
    _dimingColor = [[UIColor blackColor] colorWithAlphaComponent:0.6f];
...
- (NSInteger)addHCustomView:(UIView *)customView onCenter:(CGPoint)center
{
    JMCustomRectHole *customHole = [JMCustomRectHole new];
    customHole.holeRect = CGRectMake(center.x - customView.width/2, center.y - customView.height/2, customView.width, customView.height);
    customHole.customView = customView;
    customHole.holeType = JMHoleTypeCustomRect;
    [self.holes addObject:customHole];
    [self setNeedsDisplay];
