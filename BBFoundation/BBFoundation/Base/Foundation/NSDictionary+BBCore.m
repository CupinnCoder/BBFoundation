...
@implementation _BBXMLDictionaryParser {
...
- (void)textEnd {
    _text = _text.bb_stringByTrim.mutableCopy;
    if (_text.length) {
...
@implementation NSDictionary (BBCore)

+ (NSDictionary *)bb_dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
...
+ (NSDictionary *)bb_dictionaryWithPlistString:(NSString *)plist {
    if (!plist) return nil;
    NSData *data = [plist dataUsingEncoding:NSUTF8StringEncoding];
    return [self bb_dictionaryWithPlistData:data];
}
...
- (NSArray *)bb_allValuesSortedByKeys {
    NSArray *sortedKeys = [self bb_allKeysSorted];
...
+ (NSDictionary *)bb_dictionaryWithXML:(id)xml {
    _BBXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_BBXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_BBXMLDictionaryParser alloc] initWithData:xml];
    }
...
- (NSNumber *)bb_numverValueForKey:(NSString *)key default:(NSNumber *)def {
    if (!key) return def;
    id value = self[key];
    if (!value || value == [NSNull null]) return def;
    if ([value isKindOfClass:[NSNumber class]]) return value;
...
- (NSString *)bb_stringValueForKey:(NSString *)key default:(NSString *)def {
    if (!key) return def;
    id value = self[key];
    if (!value || value == [NSNull null]) return def;
...
@implementation NSMutableDictionary (BBCore)

+ (NSMutableDictionary *)bb_dictionaryWithPlistData:(NSData *)plist {
    if (!plist) return nil;
...
+ (NSMutableDictionary *)bb_dictionaryWithPlistString:(NSString *)plist {
    if (!plist) return nil;
    NSData *data = [plist dataUsingEncoding:NSUTF8StringEncoding];
    return [self bb_dictionaryWithPlistData:data];
}

- (id)bb_popObjectForKey:(id)aKey {
    if (!aKey) return nil;
    id value = self[aKey];
